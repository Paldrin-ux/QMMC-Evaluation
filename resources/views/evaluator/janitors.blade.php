@extends('layouts.app')
@section('page-title', 'My Assigned Janitors')

@section('content')
<div class="page-header">
  <h1>My Assigned Janitors</h1>
  <a href="{{ route('evaluator.history') }}" class="btn btn-secondary">View My Submissions →</a>
</div>
    ...
    <div class="card-body">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
        <div style="width:44px;height:44px;border-radius:50%;background:var(--navy);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:#fff;flex-shrink:0">
          {{ strtoupper(substr($janitor->name, 0, 2)) }}
        </div>
        <div>
          @php $recent = $recentEvaluationByJanitor[$janitor->id] ?? null; @endphp
          <div style="font-weight:700;font-size:14px">
            <a
              href="{{ route('evaluator.evaluate.create', $janitor) }}"
              style="text-decoration:none;color:inherit"
              class="janitor-name-link {{ $recent ? 'already-evaluated' : '' }}"
              data-already-evaluated="{{ $recent ? '1' : '0' }}"
              data-janitor-name="{{ $janitor->name }}"
              data-expires-at="{{ $recent ? $recent->created_at->copy()->addHours(24)->format('F d, Y H:i') : '' }}"
            >
              {{ $janitor->name }}
            </a>
          </div>
          @if($janitor->employee_id)
            <div style="font-size:11px;color:var(--muted)">ID: {{ $janitor->employee_id }}</div>
          @endif
        </div>
      </div>

      {{-- Areas shared between janitor and evaluator ── --}}
      <div style="margin-bottom:14px">
        <div style="font-size:11px;color:var(--muted);text-transform:uppercase;font-weight:600;margin-bottom:5px">
          Area(s)
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:4px">
          @foreach($janitor->areas->whereIn('id', $assignedAreas->pluck('id')) as $area)
      ...
      @php $last = $janitor->evaluations()->latest('eval_date')->first(); @endphp
      @if($last)
        <div style="font-size:11px;color:var(--muted);margin-bottom:12px">
          Last evaluated: <strong style="color:var(--text)">{{ $last->eval_date->format('M d, Y') }}</strong>
          · Score: <strong style="color:var(--navy-lt)">{{ $last->total_score }}/100</strong>
        </div>
      @else
      ...
      <a
        href="{{ route('evaluator.evaluate.create', $janitor) }}"
        class="btn btn-primary janitor-evaluate-btn {{ $recent ? 'already-evaluated' : '' }}"
        style="width:100%;justify-content:center {{ $recent ? 'opacity:0.65;pointer-events:auto' : '' }}"
        data-already-evaluated="{{ $recent ? '1' : '0' }}"
        data-janitor-name="{{ $janitor->name }}"
        data-expires-at="{{ $recent ? $recent->created_at->copy()->addHours(24)->format('F d, Y H:i') : '' }}"
      >
