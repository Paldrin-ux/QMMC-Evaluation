@extends('layouts.app')
@section('page-title', 'Evaluate: ' . $janitor->name)
@section('content')
<style>
...
<div class="page">
  ...
  <form method="POST" action="{{ route('evaluator.evaluate.store', $janitor) }}" id="evalForm">
    @csrf

    <div class="info-row">
      <div class="info-cell">
        <label>JANITOR'S NAME:</label>
        <input type="text" name="janitor_name" required value="{{ old('janitor_name', $janitor->name) }}">
      ...
      <div class="info-cell">
        ...
        <select name="area_id" required>
          @foreach($areas as $a)
            <option value="{{ $a->id }}" {{ (string) old('area_id', $areas->first()->id) === (string) $a->id ? 'selected' : '' }}>{{ $a->name }}</option>
          @endforeach
        </select>
        @error('area_id')<span class="error-text">{{ $message }}</span>@enderror
      </div>
      <div class="info-cell">
        <label>DATE:</label>
        <input type="date" name="eval_date" required value="{{ old('eval_date', now()->format('Y-m-d')) }}">
        @error('eval_date')<span class="error-text">{{ $message }}</span>@enderror
      </div>
      <div class="info-cell">
        <label>TIME:</label>
        <input type="time" name="eval_time" required value="{{ old('eval_time', now()->format('H:i')) }}">
        @error('eval_time')<span class="error-text">{{ $message }}</span>@enderror
      </div>
    </div>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="remarks-col">
            <textarea
              class="remarks-input"
              name="remarks_{{ $item['field'] }}"
              rows="2"
              placeholder="Remarks…">{{ old('remarks_' . $item['field']) }}</textarea>
          </td>
        </tr>
        @endforeach
      </tbody>
    ...
    <div class="sig-row">
      ...
      <div class="sig-block">
        ...
        <input type="hidden" name="sig_evaluated" id="sig_evaluated" value="{{ old('sig_evaluated') }}">
        <input type="text"
               class="sig-name-input"
               name="evaluated_by"
               value="{{ old('evaluated_by') }}"
      ...
      <div class="sig-block">
        ...
        <input type="hidden" name="sig_noted" id="sig_noted" value="{{ old('sig_noted') }}">
        <input type="text"
               class="sig-name-input"
               name="noted_by"
               value="{{ old('noted_by') }}"
    ...
    <div class="submit-row">
      ...
      <br>
      <a href="{{ route('evaluator.history') }}" class="btn-link">View All Records &rarr;</a>
    </div>
  </form>
